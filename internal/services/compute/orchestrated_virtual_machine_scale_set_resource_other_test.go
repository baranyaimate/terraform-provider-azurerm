...
func TestAccOrchestratedVirtualMachineScaleSet_otherEncryptionAtHostEnabled(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_orchestrated_virtual_machine_scale_set", "test")
	r := OrchestratedVirtualMachineScaleSetResource{}
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		data.ImportStep("os_profile.0.linux_configuration.0.admin_password"),
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("encryption_at_host_enabled").HasValue("false"),
			),
		},
		data.ImportStep("os_profile.0.linux_configuration.0.admin_password"),
		{
...
func (OrchestratedVirtualMachineScaleSetResource) otherEncryptionAtHostEnabled(data acceptance.TestData, enabled bool) string {
	r := OrchestratedVirtualMachineScaleSetResource{}
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
...
resource "azurerm_orchestrated_virtual_machine_scale_set" "test" {
  ...
  encryption_at_host_enabled = %[4]t
